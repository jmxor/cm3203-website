...
export default function PCBCModePage() {
  ...
  function startAnimation() {
    ...
    for (let i = 0; i < plaintextInput.length; i++) {
      let temp
      if (i < 8) {
        temp = stringXOR(plaintextInput[i], IVInput[i % 8])
      } else {
        temp = stringXOR(plaintextInput[i], stringXOR(plaintextInput[i-8], ciphertext[i-8]))
      }
...
function PCBCSection(props: PCBCSectionProps) {
  ...
  return (
    <div className="relative w-full shrink-0 bg-[url('../../public/pcbc-bg-0.svg')]">

      <div className="absolute top-[134px] left-[102px] w-16 h-[26px] border-2 border-black text-center align-middle font-mono">Key</div>

      <div className="absolute top-[72px] left-[9px]">
      ...
      <div className="w-[194px] flex flex-col items-center justify-center ml-auto mr-12">
        ...
        <div className="w-[98px] h-[50px] mt-[96px] mb-12 flex items-center border-2 border-black rounded font-mono">
